...
$composeDir = Join-Path $PSScriptRoot "Container\db_version1"
Push-Location $composeDir

Write-Host "Resetting dev SQL stack (containers + volumes)..." -ForegroundColor Cyan
...
Write-Host "Starting sqlserver container..." -ForegroundColor Cyan
...
Write-Host "Running one-time database init and seed job..." -ForegroundColor Cyan
$maxAttempts = 3
$seedSucceeded = $false
for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
	if ($attempt -gt 1) {
		...
		Write-Host "Restarting sqlserver before retry..." -ForegroundColor Cyan
		docker compose up -d sqlserver
		if ($LASTEXITCODE -ne 0) {
...
Write-Host "Database is ready for development on localhost,11433." -ForegroundColor Green
if ($ForegroundLogs) {
	Write-Host "Following sqlserver logs (Ctrl+C to stop viewing logs)..." -ForegroundColor DarkGray
	docker compose logs -f sqlserver
}
else {
	Write-Host "Use 'docker compose -f .\Container\db_version1\docker-compose.yml logs -f sqlserver' to view SQL logs when needed." -ForegroundColor DarkGray
}
